...
현재 버전은 **v0.2.0 / AI 설명 기능을 포함한 출품 준비 상태**입니다.
...
반복결제 후보의 `AI 설명` 버튼을 누르면 결제 원문 대신 브라우저에서 추출한 요약값만
Vercel 서버 함수를 통해 AI에 전달합니다. AI는 통합된 결제처와 자동 분류, 후보로 판단한
이유, 해지 우선순위, 월·연 지출 영향을 짧게 설명합니다.

- 원문 결제내역은 AI 요청에 포함하지 않음
- API 키는 브라우저에 노출하지 않고 Vercel 환경변수로만 사용
- AI 연결이 없거나 실패하면 로컬 자동 설명으로 대체
- AI 분석은 사용자가 버튼을 눌렀을 때만 실행되며 하루 5회 한도와 사용량을 표시
